...
export const getWeatherForeCast = async function getCurrentWeatherForecast(lat, lon) {
    try {
        const from = getCurrentTime(), until = getFutureDate(1)
...
export const getWeatherForeCast10Days = async function getWeatherForeCast10Days(lat, lon) {
    try {
        const from = getCurrentTimeBeginningOfDay(), until = getFutureDate(10)
        const response = await axios.get(`https://api.brightsky.dev/weather?date=${from}&last_date=${until}&lat=${lat}&lon=${lon}`)
        return response.data
    } catch (error) {
        console.log("Something went wrong when getting the current weather")
...
function getCurrentTimeBeginningOfDay() {
    const now = new Date()
    const year = now.getFullYear()
    const month = String(now.getMonth() + 1).padStart(2, '0')
    const day = String(now.getDate()).padStart(2, '0')
